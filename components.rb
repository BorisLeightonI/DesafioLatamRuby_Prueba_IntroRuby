...
<footer class=\"text-center text-light p-1\" style =\"background : #1c4e80\">
   ...
   <a href=\"http://www.github.com/borisleightoni\"><i class=\"fab fa-github fa-3x text-white mb-3\"></i></a>
   <a href=\"
   ...
   \"><i class=\"fab fa-linkedin fa-3x ml-2 mb-3 text-white\"></i></a>
   <p>DesafioLatam 2020</p>
...
def content(x,y)
    data = request(x,y)
    camaras_fotos = cuenta_fotos(data) 

    if data.nil?
        cards = '<h3 class="text-center my-5">No hay datos disponibles</h3>
        '
    else  
        cards = '<div class="text-center mb-3 border">'
        camaras_fotos.each do |k,v| 
        
            cards += "Cámara:#{k} tiene #{v} fotos  "
...
def cuenta_fotos(datos)
    res = {}
    faz = raz = mast = chem = mali = mardi = navcam = pancam = mini = 0
...
#    puts "FHAZ: #{faz} RHAZ: #{raz} MAST: #{mast}"
    if faz!=0 then res[:FHAZ] = faz end
    if raz!=0 then res[:RHAZ] = raz end
    if mast!=0 then res[:MAST] = mast end
    if chem!=0 then res[:CHEMCAM] = chem end
    if mali!=0 then res[:MAHLI] = mali end
    if mardi!=0 then res[:MARDI] = mardi end
    if navcam!=0 then res[:NAVCAM] = navcam end
    if pancam!=0 then res[:PANCAM] = pancam end
    if mini!=0 then res[:MINITES] = mini end
